...
async function claim(): Promise<Claim | null> {
	const res = await post("/v1/work/claim", { worker_id: WORKER_ID, labels });
...
async function runClaim(work: Claim): Promise<void> {
	...
	try {
		output = await executeStep(work.kind, work.spec, work.session);
	} catch (error: any) {
